...
USER_STORE_PATH = Path("data/user_store.json")
...
def load_local_store():
    try:
        if USER_STORE_PATH.exists():
            return ensure_store_shape(json.loads(USER_STORE_PATH.read_text(encoding="utf-8")))
    except Exception:
        pass
...
def render_login(store):
    ...
    st.caption("Banco v7.08 · reinstalación completa limpia.")
